...
public class AtualizaEfetivacaoAgendamentoPixTest extends BaseTeste {
	...
	private static final Long NSU_TRANSACAO = 360L;
...
//	private static final String URI = "https://simtx.webservices.des.caixa/pagamentos-instantaneos/v1/pagamentos";
	...
	public String executarRequisicao(String path, String token, String nsuTransacao) {
		if(UTILIZARHTTP) {
			String jsonPagamento = recuperarJson(BASE_PATH_JSON.concat(PATH).concat("agendamento/atualizacao/").concat(path));
			jsonPagamento = jsonPagamento.replace("{DATATRANSACAO}", "2021-06-28");
			
			ClientResponse resposta = RequisicaoHTTP.requestPutApi(URI.concat(nsuTransacao), token, jsonPagamento);
	...
	public void testSucesso() throws InterruptedException {
		...
		executarRequisicao(pathTeste001, token, "123");
	...
	@Test
	public void testSucessoComQrCode() throws InterruptedException {
		String pathTeste001 = "V1/sucessoQrCode.json";
		List<Long> tarefas = new ArrayList<>();
		tarefas.addAll(Arrays.asList(TAREFAS_NEGOCIAIS));

		executarRequisicao(pathTeste001, token, "360");
	...
	public void testRJCT() {
		...
		String saida = executarRequisicao(pathTeste001, token, "123");
