...
//		tokenTransacao = tokenTransacaoSaida.getToken();
	...
	public String executarRequisicao(String path, String token, String meioEntrada, String nsuTransacao, int statusEsperado) {
		if(UTILIZARHTTP) {
			...
			assertEquals(Constantes.RESPOSTA_DIVERGENTE_HTTP + resposta.getStatus() + "] " + resposta,
					statusEsperado, resposta.getStatus());
	...
	public void testIBCSucesso() throws IOException {
		...
		String saida = executarRequisicao(pathTeste001, token, meioEntrada, nsuTransacao, Response.Status.CREATED.getStatusCode());
	...
	public void testNBMSucesso() throws IOException {
		try {
			...
			String saida = executarRequisicao(pathTeste001, tokenTransacao, meioEntrada, nsuTransacao, Response.Status.CREATED.getStatusCode());
	...
	public void testNsuNaoEncontrado() throws IOException {
		...
		String nsuTransacao = "1";

		String meioEntrada = recuperarJson(BASE_PATH_JSON.concat(PATH_MEIO_ENTRADA).concat("assinatura_simples/dados.json"));
		meioEntrada = codificaBase64(meioEntrada);

		String saida = executarRequisicao(pathTeste001, token, meioEntrada, nsuTransacao, Response.Status.BAD_REQUEST.getStatusCode());
		assertFalse(Constantes.RESPOSTA_CONTEM_CAMPOS_NULOS, saida.contains("null"));
