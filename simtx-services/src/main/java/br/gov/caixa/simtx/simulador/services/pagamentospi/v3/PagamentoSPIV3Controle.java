...
public class PagamentoSPIV3Controle extends Controle {

	private static final Logger logger = Logger.getLogger(PagamentoSPIV3Controle.class);
	...
	private RequisicaoPagamentoSpiV3 requisicao;
	...
	@POST
	@Path("/v3/ordens-pagamento")
	public Response validarServico(@Context HttpServletRequest httpRequest, String json) {
		try {
			requisicao = mapper.readerFor(RequisicaoPagamentoSpiV3.class).readValue(json);
	...
	@Asynchronous
	public void executarAtualizaV3() {
		try {
			Thread.sleep(5000);
