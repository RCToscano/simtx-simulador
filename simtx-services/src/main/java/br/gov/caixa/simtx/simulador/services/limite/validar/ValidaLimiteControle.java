...
public class ValidaLimiteControle extends Controle {

	private static final Logger logger = Logger.getLogger(ValidaLimiteControle.class);
	
	private static final String PATH = "valida_limite";
	

	@POST
	@Path("/v1/{conta}/validar")
	public Response validarServico(@Context HttpServletRequest httpRequest, @PathParam("id_conta") String conta, String json) {
		try {
			RequisicaoValidaLimite requisicao = gson.fromJson(json, RequisicaoValidaLimite.class);
