...
public class ValidaPermissaoControle extends Controle {
	...
	public Response validarServico(@Context HttpServletRequest httpRequest, @PathParam("cpf") String cpf, RequisicaoValidaPermissao requisicao) {
		try {
			...
			return Response.status(Response.Status.BAD_REQUEST)
					.entity(montarMsgErro(BASE_PATH_JSON + PATH + "/400.json", e.getMensagem())).build();
		} 
		catch (Exception e) {
			logger.error(e);
