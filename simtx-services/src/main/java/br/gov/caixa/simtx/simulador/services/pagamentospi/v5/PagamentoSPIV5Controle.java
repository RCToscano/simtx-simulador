...
import br.gov.caixa.simtx.simulador.services.pagamentospi.v2.TipoPrioridadePagamentoEnum;
...
public class PagamentoSPIV5Controle extends Controle {

	private static final Logger logger = Logger.getLogger(PagamentoSPIV5Controle.class);
	...
	private RequisicaoPagamentoSpiV5 requisicao;
	...
	@POST
	@Path("/v5/ordens-pagamento")
	public Response validarServico(@Context HttpServletRequest httpRequest, String json) {
		try {
			requisicao = mapper.readerFor(RequisicaoPagamentoSpiV5.class).readValue(json);
	...
	@Asynchronous
	public void executarAtualizaV4() {
		try {
			Thread.sleep(5000);
			logger.info("[SIMULADOR] Executando Atualiza Pagamento Instantaneo V5");
			RequisicaoAtualizacao atualizacao = new RequisicaoAtualizacao();
			atualizacao.setDataTransacao(DataUtil.getDataFormatada(new Date(), DataUtil.FORMATO_DATA_XML));
			atualizacao.setDataHora(DataUtil.getDataFormatada(new Date(), DataUtil.FORMATO_DATA_YYYY_MM_DD_HIFEN_JSON));
			atualizacao.setIdFimAFim(requisicao.getIdFimAFim());
			atualizacao.setValorTarifa(BigDecimal.TEN);
			...
			if (requisicao.getInformacoesEntreUsuarios() == null || requisicao.getInformacoesEntreUsuarios() != null
				...
				atualizacao.setPaymentId(requisicao.getPaymentId());
			...
			String[] url = uri.split("/");
			uri = url[0].concat("//").concat(url[2]).concat(PACOTE + "/v4/pagamentos/").concat(requisicao.getNsu().toString());
